...
MT = {
        ...
        options.success = function (data) {
          if (data.token) {
            if (data.token.type == 0) {
              ...
              if (data.token.TokenState == "100" ||
                data.token.TokenState == "110") {
                success(data);

              } else if (data.token.TokenState == "101" ||
                data.token.TokenState == "111") {
                //更新
                MT.locCache_set('s_token', data.token.NewS_token, data.token.StokenExpires + 36, "LS");
                success(data);
          ...
          } else if (data.id === 600) { //需要发送验证验证码---afei
  ...
  Logo: function () {
    if (typeof global === 'object' && global.Logo) {
  ...
  var eleObj = {
    ...
    on: {
      click: function (event) {
        if ($(event.target).attr('id') === 'MTCodeMessageBox') {
          $(this).hide(); //隐藏
        }
      }
    },
        ...
        },
        {
          ...
          child: [{
            ...
            },
            {
              ...
              on: {
                click: function () { //提交验证码
                  var code = $(".inputBox>input").val();
...
function renderFun(eleObj) {
  ...
  if (typeof eleObj.attr === "object") { //设置属性
    for (var key in eleObj.attr) {
      if (key === 'style') {
        temBox.css(eleObj.attr[key])
      } else {
        temBox.attr(key, eleObj.attr[key]);
      }
    }
  }
  if (typeof eleObj.on === "object") { //绑定事件
    for (var key in eleObj.on) {
      temBox.on(key, eleObj.on[key])
    }
  }
  if (typeof eleObj.text === "string") { //设置文本
    temBox.text(eleObj.text)
  }
  if (eleObj.child && eleObj.child.length > 0) { //设置子属性
    eleObj.child.forEach(function (item) {
      temBox.append(renderFun(item));
    })
  }
  return temBox;
}
