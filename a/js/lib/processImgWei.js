 ...
 * processImgWei.js  
 * 
 * 这是一个基于微信JSSDK canvas，应用在移动端的前端图片压缩的JS 
 * Date: 2017-04-02
 */
                ...
                if (wx) {
                    ...
                    wx.chooseImage({
                        ...
                        success: function (res) {
                            ...
                            var ImagesId = res.localIds;
                            var srcString = "";
                            if (ImagesId && ImagesId.length > 0) {
                                srcString = ImagesId[0];
                            }
                       
                        //提交后台服务器后下载
                            if (UseWeiXinSDKUpdatePicAfterMethod) {
                              
                            

                                viewImg.src = srcString;

                                wx.uploadImage({
                                    localId: ImagesId.toString(), // 需要上传的图片的本地ID，由chooseImage接口获得
                                    isShowProgressTips: 1, // 默认为1，显示进度提示
                                    success: function (sres) {

                                        var media_id = sres.serverId; // 返回图片的服务器端ID  
                                    ...
                                    fail: function (res2) {
                                        viewImg.src = imgSrc;
                                        alert("上传图片失败：" + JSON.stringify(res2));
                                    }
                                });
                                ...
                                try {
                                    ...
                                    success: function (sres) {
                                        ...
                                        var media_id = sres.serverId; // 返回图片的服务器端ID  
                                        TipLoad.loading('正在显示图片,请稍后...');
                                        // Create XHR, BlobBuilder and FileReader objects
                                        var xhr = new XMLHttpRequest(),
                                        fileReader = new FileReader();
                                        xhr.open("GET", "/ServiceAPI/usercenter/Manager.aspx?action=GetMediaUrl&media_id=" + media_id, true);
                                        ...
                                        xhr.addEventListener("load", function () {
                                            ...
                                            if (xhr.status === 200) {
                                                ...
                                                fileReader.onload = function (evt) {
                                                    ...
                                                    result = "data:image/jpeg;base64," + result.split(',')[1];
                                                    console.log(result);
                                                    // Set image src to Data URL
                                                    viewImg.setAttribute("src", result);
