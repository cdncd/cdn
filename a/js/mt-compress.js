...
var Base64StringToImage = function(opt) {
  ...
  var li = opt.li;
  console.log("li,", li);
  console.log("ImgDirectory:", ImgDirectory);
  ...
  try {
    ...
    xhr.upload.onprogress = function(event) {
      var url = cycle
        .update(Math.round((event.loaded / event.total) * 100))
        .GetImage();
      ...
      (function(inputBox) {
        inputFile.addEventListener(
          ...
          function() {
            ...
            var viewLength = inputBox.parentNode.getElementsByClassName(
              opt.viewClass
            ).length;
            ...
            wx.chooseImage({
              ...
              success: function(res) {
                ...
                var liIndex;
                if (opt.model == 2) {
                  liIndex = Array.prototype.indexOf.call(
                    inputBoxList,
                    inputBox
                  );
                  if (!!opt.fileName[liIndex]) {
                    ...
                    alert(
                      "model为2的情况下，fileName必须为一个长度和上传按钮数一致的数组"
                    );
                    return;
                  }
                } else {
                  liIndex = inputBoxList[0].parentNode.getElementsByClassName(
                    opt.viewClass
                  ).length;
                ...
                try {
                  if (opt.checkResult.getLocalImgData) {
                    ...
                    alert(
                      "您的微信不支持,请升级您的微信\b\r" + navigator.userAgent
                    );
                  }
                } catch (ex) {
                  if (callback != undefined) {
        ...
        inputFile.onchange = function(e) {
          ...
          if (opt.revise) {
            if (opt.model === 2) {
              ...
              li = this.parentNode.parentNode.getElementsByClassName(
                opt.viewClass
...
function checkMax(opt) {
  ...
  var viewLength = inputBox.parentNode.querySelectorAll("." + opt.viewClass)
    .length;
  if (viewLength === opt.maxCount) {
...
function compressor(opt) {
  ...
  if (opt.viewClickEvent === "enlarge") {
    ...
    divMark.setAttribute(
      "style",
      "position: fixed;top: 0;bottom: 0;left: 0;right: 0;background-color: rgba(0, 0, 0, .5);display: none;z-index: 99;"
    );
    divMark.innerHTML =
      '<div id="enlarge" style="width: 80%;margin: 30px auto;position: relative;"><img src="" alt="" style="display: block;width:100%;height:' +
      (window.screen.height - 200) +
      'px;"><span id="del" style="position: absolute;bottom: -22px;width: 50%;text-align: center;cursor: pointer;color: #333;border-radius: 5px;left: 0;background-color: rgb(250, 125, 42);">删除</span><span id="revise" style="position: absolute;bottom: -22px;width: 50%;text-align: center;cursor: pointer;color: #333;border-radius: 5px;right: 0;background-color: rgb(115, 255, 0);">修改<input type="file" accept="image/*" style="opacity:  0;position:  absolute;left:  0;right:  0;top: 0;bottom:  0;width:  100%;height:  100%;"></span></div>';
    ...
    divMark.addEventListener("click", function(e) {
      if (e.target.id === "del") {
        if (confirm("确认删除图片吗？")) {
          ...
          var viewList = inputBoxList[0].parentNode.querySelectorAll(
            "." + opt.viewClass
          );
          if (opt.model == 2) {
            ...
            for (var i = viewIndex; i < viewList.length; i++) {
              viewList[i].getElementsByTagName("input")[0].name =
                opt.prefix + (i - 1);
            }
    ...
    divMark
      ...
      .addEventListener("change", function(e) {
        ...
        opt.revise = true;
        if (opt.model == 2) {
          inputBoxList[divMark.dataset.index]
            .getElementsByTagName("input")[0]
            .onchange(e);
        } else {
          inputBoxList[0].getElementsByTagName("input")[0].onchange(e);
  ...
  inputBoxList[0].parentNode.addEventListener("click", function(e) {
    ...
    if (opt.viewClickEvent === "enlarge") {
      ...
      opt.divMark
        ...
        .setAttribute("src", e.target.dataset.imgsrc);
      console.log(e.target.dataset.index);
      opt.divMark.setAttribute("data-index", e.target.dataset.index);
      opt.e = e;
    } else if (opt.viewClickEvent === "del") {
      //删除图片
      if (confirm("确认删除图片吗？")) {
        if (opt.model == 2) {
          console.dir(e.target);
          var liIndex = e.target.dataset.index;
          document.getElementsByClassName(opt.inputBoxClass)[
            liIndex
          ].style.display = "block";
